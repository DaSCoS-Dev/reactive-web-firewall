...
# SCOPO / PURPOSE
#   Rileva sonde verso l’endpoint REST batch/v1 di WordPress.
#   Detects probes against the WordPress REST batch/v1 endpoint.
#
# CONFIGURAZIONE / CONFIGURATION
#   Abilitazione e durata del ban non si modificano qui. Usare:
#     policy_wp_batch = off | permanent | 30m | 4h | 3d | 2w ...
...
#   perl -c /etc/reactive-web-firewall/rules.d/020-wordpress-batch.pm
...
return {
    name => 'wp_batch',
    description => 'WordPress REST batch/v1 probe',
    priority => 20,
    default_policy => '3d',
    detect => sub {
        my ($c) = @_;
        my $u = $c->{decoded_uri};
        return 'wordpress-batch-v1'
            if $u =~ m{^/wp-json/batch/v1(?:/|$|[?#])}
            || $u =~ m{(?:\?|[&;])rest_route=/batch/v1(?:/|$|[&#;])};
        return undef;
    },
};
